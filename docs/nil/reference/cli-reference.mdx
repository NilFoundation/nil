...
```bash
nil_cli contract call-readonly CONTRACT_ADDRESS CALLDATA | METHOD_NAME [--abi path/to/contract.abi] [--fee-credit FEE_CREDIT]
```
...
```bash
nil_cli send-tokens ADDRESS [--fee-credit FEE_CREDIT] [--no-wait] [--token stringArray]
```
...
```bash
nil_cli wallet send-message ADDRESS BYTECODE | METHOD [ARGS] [--abi path/to/contract.abi] [--amount AMOUNT] [--fee-credit FEE_CREDIT] [--no-wait] [--token TOKENS]
```
