...
`FeeCredit` is an estimate created by the message sender rather than a precisely calculated message execution fee.
...
When percentage forwarding is used in mixed forwarding, it is calculated first. For example, if there is `900_000` leftover `FeeCredit` and `300_000` is forwarded by value to other contracts while 30% is forwarded by percentage, 30% would still equal `300_000`. 

Conversely, equal split forwarding is always calculated last. In the above example, only `300_000` leftover `FeeCredit` will be left after calculating value and percentage forwarding. This amount will be split equally among the remaining contracts to be called.
