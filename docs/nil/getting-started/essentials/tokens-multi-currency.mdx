...
```bash
nil minter create-currency OWNER_ADDRESS AMOUNT
```

To send an existing currency (with some amount already minted) from a minter to another contract:

```bash
nil minter withdraw-currency OWNER_ADDRESS AMOUNT RECEIVER_ADDRESS
```
...
```bash
nil wallet new
nil wallet new --salt 1
```

Create a new currency for Wallet 1 and withdraw it:

```bash
nil minter create-currency WALLET_ONE_ADDRESS 50000 token --withdraw
```

Create a new currency for Wallet 2 and keep it in the minter:

```bash
nil minter create-currency WALLET_TWO_ADDRESS 30000 another-token
```

Withdraw the second currency to Wallet 1:

```bash
nil minter withdraw-currency WALLET_TWO_ADDRESS 15000 WALLET_ONE_ADDRESS
```

Check the currencies for Wallet 1:

```bash
nil contract currencies WALLET_ONE_ADDRESS
```
