...
```ts file=../../../tests/tokens-multi-currency-support.test.mjs start=startNilJSMintingExample end=endNilJSMintingExample
```
...
```ts showLineNumbers file=../../../tests/tokens-multi-currency-support.test.mjs start=startNilJSCreationTutorialAdvanced end=endNilJSCreationTutorialAdvanced
```

Create a new currency for Wallet 2 and send it to Wallet 1:

```ts showLineNumbers file=../../../tests/tokens-multi-currency-support.test.mjs start=startNilJSCreationTutorialWalletTwo end=endNilJSCreationTutorialWalletTwo
```

Check the currencies for Wallet 1:

```ts showLineNumbers file=../../../tests/tokens-multi-currency-support.test.mjs start=startNilJSBalancesCheck end=endNilJSBalancesCheck
```