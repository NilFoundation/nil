...
This tutorial focuses on the =nil; CLI. To learn more about using `Nil.js` to create a wallet, deploy smart contracts, and send messages to these contracts, refer to the [**Cookbook**](../cookbook/niljs-deploy/deploy-call-smart-contract.mdx).
...
```bash file=../../tests/commands.mjs start=startSaltWalletCreation end=endSaltWalletCreation
```
