...
Messages in =nil; can either be incoming or outgoing:

* Incoming messages trigger contract execution.
* Outgoing messages are spawned by contracts as part of execution. These outgoing messages are sent to other contracts located on various shards.

To support [**cross-shard communications**](../getting-started/essentials/handling-async-execution.mdx), execution shards must record all necessary outgoing messages posted in other execution shards.
...
sequenceDiagram
    ...
    S2 ->> S2: Shard 2 creates a new block that includes the picked up messages
```
