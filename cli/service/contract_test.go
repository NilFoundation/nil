...
func TestRunContract_Successfully(t *testing.T) {
	...
	txHash, err := service.RunContract(types.EmptyAddress, nil, types.EmptyAddress)
	require.NoError(t, err)
...
func TestRunContract_Err(t *testing.T) {
	...
	_, err := service.RunContract(types.EmptyAddress, nil, types.EmptyAddress)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "RPC error")
}
...
func TestDeployContract_Successfully(t *testing.T) {
	...
	txHash, _, err := service.DeployContract(types.BaseShardId, types.EmptyAddress, "0x6001600101600055")
	require.NoError(t, err)
	assert.Equal(t, mockSuccessHash, common.HexToHash(txHash))
}
...
func TestContractDeployment_Err(t *testing.T) {
	...
	_, _, err := service.DeployContract(types.BaseShardId, types.EmptyAddress, "0x6001600101600055")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "RPC error")
}
