...
export async function setMockCounterpartyInBridges(networkName: string) {
    ...
    if (!isValidAddress(config.l1MockContracts.mockL2Bridge)) {
    ...
    const tx = await l1ERC20BridgeInstance.setCounterpartyBridge(config.l1MockContracts.mockL2Bridge);
    await tx.wait();
    ...
    const tx2 = await l1ETHBridgeInstance.setCounterpartyBridge(config.l1MockContracts.mockL2Bridge);
    await tx2.wait();
