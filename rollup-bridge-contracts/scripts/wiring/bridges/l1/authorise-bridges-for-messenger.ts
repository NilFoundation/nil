...
export async function authoriseBridges(networkName: string) {
    ...
    console.log(`authorising l1-bridges: ${[config.l1ERC20BridgeConfig.l1ERC20BridgeProxy, config.l1ETHBridgeConfig.l1ETHBridgeProxy]} to engage with l1BridgeMessenger: ${config.l1BridgeMessengerConfig.l1BridgeMessengerProxy}`);
    const tx = await l1BridgeMessengerInstance.authorizeBridges(
        [config.l1ETHBridgeConfig.l1ETHBridgeProxy, config.l1ERC20BridgeConfig.l1ERC20BridgeProxy]);
