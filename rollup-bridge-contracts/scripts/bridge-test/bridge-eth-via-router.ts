...
export async function bridgeETHViaRouter() {
    ...
    const tx = await l1BridgeRouterInstance.depositETH(recipientAddress, eth_amount, l2FeeRefundRecipientAddress, gasLimit, userFeePerGas, userMaxPriorityFeePerGas, { value: total_native_amount });
    await tx.wait();
