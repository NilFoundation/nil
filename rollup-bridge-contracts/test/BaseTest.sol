...
import { L1BridgeMessenger } from "../contracts/bridge/l1/L1BridgeMessenger.sol";
...
contract BaseTest is Test {
  ...
  ProxyAdmin public l1BridgeMessengerProxyAdmin;
  ...
  L1BridgeMessenger public l1BridgeMessengerImplementation;

  address public l1BridgeMessengerProxyAdminAddress;

  L1BridgeMessenger public l1BridgeMessengerProxyInstance;
  ...
  function setUp() public virtual {
    ...
    publicDataInfoMock = INilRollup.PublicDataInfo({
    ...
    l1BridgeMessengerImplementation = new L1BridgeMessenger();
    address l1BridgeMessengerProxyAddress = _deployL1BridgeMessengerProxy(address(l1BridgeMessengerImplementation));
    l1BridgeMessengerProxyInstance = L1BridgeMessenger(payable(l1BridgeMessengerProxyAddress));
    ...
    l1BridgeMessengerProxyInstance.initialize(_owner, _defaultAdmin, address(rollup), 1000);

    rollup.setL1BridgeMessenger(address(l1BridgeMessengerProxyInstance));

    l1BridgeMessengerProxyAdminAddress = CustomTransparentUpgradeableProxy(payable(l1BridgeMessengerProxyAddress))
      .getAdmin();

    l1BridgeMessengerProxyAdmin = ProxyAdmin(l1BridgeMessengerProxyAdminAddress);
    ...
    l1BridgeMessengerProxyAdmin.upgradeAndCall(
      ITransparentUpgradeableProxy(address(l1BridgeMessengerProxyInstance)),
      address(l1BridgeMessengerImplementation),
      new bytes(0)
    );
    vm.stopPrank();
  }
  ...
  function _deployL1BridgeMessengerProxy(address _logic) internal returns (address) {
    if (_logic == address(0)) _logic = address(new L1BridgeMessenger());
    CustomTransparentUpgradeableProxy proxy = new CustomTransparentUpgradeableProxy(_logic, _owner, new bytes(0));
    return address(proxy);
  }
}
