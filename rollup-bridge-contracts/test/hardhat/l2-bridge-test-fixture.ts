...
import { prepareNilSmartAccountsForUnitTest } from "../../task/nil-smart-account";

export interface L2BridgeTestFixtureResult {
    ownerSmartAccount: SmartAccountV1;
    depositRecipientSmartAccount: SmartAccountV1;
    feeRefundSmartAccount: SmartAccountV1;
    l2EthBridgeVaultProxyAddress: string;
    l2BridgeMessengerProxyAddress: string;
    l2ETHBridgeProxyAddress: string;
    l2EnshrinedTokenBridgeProxyAddress: string;
    l2BridgeMessengerProxyInstance: any;
    l2ETHBridgeVaultProxyInstance: any;
    l2ETHBridgeProxyInstance: any;
}

export async function setupL2BridgeTestFixture(): Promise<L2BridgeTestFixtureResult> {
    ...
    try {
        ...
        if (!isL2EnshrinedTokenBridgeAuthorised) {
            throw new Error(`❌ L2EnshrinedTokenBridge: ${l2EnshrinedTokenBridgeProxyAddress} is not authorised on L2BridgeMessenger: ${l2BridgeMessengerProxyAddress}`);
        }
        ...
        if (!isL2ETHBridgeAuthorised) {
            throw new Error(`❌ L2ETHBridge: ${l2ETHBridgeProxyAddress} is not authorised on L2BridgeMessenger: ${l2BridgeMessengerProxyAddress}`);
        }

    } catch (err) {
        throw new Error(`❌ Error caught while getting an instance of L2BridgeMessenger: ${l2BridgeMessengerProxyAddress} , Error: ${err} `);
    }
    ...
    return {
        ...
        l2EthBridgeVaultProxyAddress,
        l2BridgeMessengerProxyAddress,
        l2ETHBridgeProxyAddress,
        l2EnshrinedTokenBridgeProxyAddress,
        l2BridgeMessengerProxyInstance,
        l2ETHBridgeVaultProxyInstance,
        l2ETHBridgeProxyInstance
    };
}
