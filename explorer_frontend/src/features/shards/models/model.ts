...
export const $shardsGasPriceMap = explorerShardsList.createStore<
  Record<string, bigint>
>({});

export const $busiestShard = $shardsGasPriceMap.map((shardsGasPriceMap) => {
  const maxGasPriceValue = Math.max(
    ...Object.values(shardsGasPriceMap).map(Number)
  );
...
export const $bestShard = $shardsGasPriceMap.map((shardsGasPriceMap) => {
  const minGasPriceValue = Math.min(
    ...Object.values(shardsGasPriceMap).map(Number)
  );
  return Object.entries(shardsGasPriceMap).find(
...
export const fetchShrdsGasPriceFx = createEffect<
  void,
  Record<string, bigint>,
  Error
>();
