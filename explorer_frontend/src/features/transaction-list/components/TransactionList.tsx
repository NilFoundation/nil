...
import { $transactionList, type TransactionListProps, fetchTransactionListFx, showList } from "../model";
...
export const TransactionList = ({ type, identifier, view }: TransactionListProps) => {
  ...
  const mappedTransactions = useMemo(() => {
    return transactions
      ...
      .map(({ from, to, hash, method, shard_id, block_id, value, success, fee_credit, flags, outgoing }) => {
        return [
          <div
            key={hash}
            className={css(
          ...
          <Link to={blockRoute} params={{ shard: shard_id, id: block_id }} key={shard_id + block_id + hash}>
  ...
  return (
    ...
    >
      <InfoContainer title={isMobile ? "Messages" : undefined}>
