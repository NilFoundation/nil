...
export const $contractWithState = combine(
	$contracts,
	$state,
	(contracts, state) => {
...
export const $activeAppWithState = combine(
	$activeApp,
	$contracts,
	(activeApp, contracts) => {
		if (activeApp === null) {
		...
		const contract =
			contracts.find((contract) => contract.bytecode === bytecode) || null;
...
>(async ({ abi, functionName, args, wallet, address, value, tokens }) => {
	...
	await waitTillCompleted(
		wallet.client,
		getShardIdFromAddress(wallet.getAddressHex()),
		hash,
	);
