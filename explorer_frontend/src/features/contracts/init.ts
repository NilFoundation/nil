...
$state.on(
	...
	(state, { address, app }) => {
		...
		const addresses = state[app.bytecode]
			? [...state[app.bytecode], address]
			: [address];
		return {
			...state,
			...
			for (const input of abiConstructor.inputs) {
				...
				switch (true) {
					case input.type === "string": {
						value = value =
							input.name && input.name in args ? args[input.name] : "";
						break;
					}
					...
					case input.type === "bool": {
						value =
							input.name && input.name in args ? !!args[input.name] : false;
						break;
					}
					...
					case input.type.slice(0, 3) === "int": {
						value =
							input.name && input.name in args ? BigInt(args[input.name]) : 0n;
						break;
					}
					default: {
						value = value =
