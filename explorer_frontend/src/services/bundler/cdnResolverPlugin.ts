  ...
  return {
    ...
    setup(build: esbuild.PluginBuild) {
      ...
      build.onResolve({ filter: /^[^./]|^\.[^./]|^\.\.[^/]/ }, async (args) => {
        ...
        if (
          args.path.match(
            /^(node:)?fs|path|crypto|buffer|stream|zlib|util|os|child_process|worker_threads|cluster$/,
          )
        ) {
      ...
      build.onResolve({ filter: /^\./, namespace: "unpkg" }, (args) => {
        ...
        if (args.path.startsWith("./") || args.path.startsWith("../")) {
          ...
          const baseUrl = importerUrl.href.endsWith(".js")
            ? importerUrl.href.substring(0, importerUrl.href.lastIndexOf("/") + 1)
            : importerUrl.href;
      ...
      build.onLoad({ filter: /.*/, namespace: "extContract" }, async (args) => {
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to fetch ${args.path}: ${response.status} ${response.statusText}`,
            );
          }
