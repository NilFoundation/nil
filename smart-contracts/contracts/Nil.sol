...
library Nil {
    ...
    uint public constant ASYNC_REQUEST_MIN_GAS = 300_000;
    ...
    function isRelayerAddress(address addr) internal pure returns (bool) {
        uint160 addrInt = uint160(addr) & uint160(0x00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000);
        return uint160(0x3333333333333333333333333333333333330000) == addrInt;
    }
...
contract NilBase {
    ...
    modifier onlyResponse() {
        require(_callerIsRelayer(), "IS_RESPONSE_TRANSACTION call failed");
        _;
    }
    ...
    modifier onlyRelayer() {
        require (_callerIsRelayer(), "Only Relayer can call this function");
        _;
    }
    ...
    function _callerIsRelayer() internal view returns (bool) {
        return msg.sender == Nil.getRelayerAddress();
    }
