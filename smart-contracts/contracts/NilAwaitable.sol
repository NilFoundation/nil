...
contract NilAwaitable is NilBase {
    ...
    function sendRequestWithTokens(
        ...
        ctrl.await_id += 1;
        // console.log("sendRequest: address=%_ context=%_ await_id=%_", address(this), context.length, ctrl.await_id);
        ctrl.awaiters[ctrl.await_id] = Awaiter({callback: cb, answer_id: ctrl.await_id, active: true, context: context});
        __Precompile__(address(0xfd)).precompileAsyncCall{value: value}(false, Nil.FORWARD_REMAINING, dst, zeroAddress,
    ...
    function onFallback(uint256 answer_id, bool success, bytes memory response) external payable {
        ...
        // console.log("onFallback: address=%_ await_id=%_ context=%_ response=%_", address(this), answer_id,
        //     awaiter.context.length, response.length);
