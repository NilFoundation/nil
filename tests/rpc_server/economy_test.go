...
func (s *SuiteEconomy) TestForwardKindMatch() {
	...
	s.Require().NoError(err)
	data = s.CallGetter(s.testAddress2, data)
	unpackedRes, err := s.abiTest.Unpack("getForwardKindRemaining", hexutil.FromHex(string(data)))
	s.Require().NoError(err)
	...
	s.Require().NoError(err)
	data = s.CallGetter(s.testAddress2, data)
	unpackedRes, err = s.abiTest.Unpack("getForwardKindPercentage", hexutil.FromHex(string(data)))
	s.Require().NoError(err)
	...
	s.Require().NoError(err)
	data = s.CallGetter(s.testAddress2, data)
	unpackedRes, err = s.abiTest.Unpack("getForwardKindValue", hexutil.FromHex(string(data)))
	s.Require().NoError(err)
	...
	s.Require().NoError(err)
	data = s.CallGetter(s.testAddress2, data)
	unpackedRes, err = s.abiTest.Unpack("getForwardKindNone", hexutil.FromHex(string(data)))
	s.Require().NoError(err)
