...
func (suite *SuiteRpc) TestDeployContractViaFaucet() {
	...
	msgHash, err := suite.client.TopUpViaFaucet(walletAddr, value)
	suite.Require().NoError(err)
	...
	suite.Require().NoError(err)
	suite.Require().Less(balance.Uint64(), value.Uint64())
...
func (suite *SuiteRpc) TestTopUpViaFaucet() {
	...
	address, receipt := suite.deployContractViaMainWallet(types.BaseShardId, types.BuildDeployPayload(walletCode, common.EmptyHash), types.Value{})
	...
	testTopUp := func(initialValue, value, expectedValue uint64, delta float64) {
		...
		mshHash, err := suite.client.TopUpViaFaucet(address, types.NewValueFromUint64(value))
		suite.Require().NoError(err)
