...
func (s *SuiteRpc) createWalletViaFaucet(ownerPrivateKey *ecdsa.PrivateKey, value int64) types.Address {
	...
	res := s.waitForReceiptOnShard(types.BaseShardId, resHash)
...
func (s *SuiteRpc) TestCreateWalletViaFaucet() {
...
func (s *SuiteRpc) TestDeployContractViaFaucet() {
	userPrivateKey, err := crypto.GenerateKey()
	s.Require().NoError(err)
	...
	s.Require().NoError(err)
	receipt := s.waitForReceiptOnShard(walletAddr.ShardId(), msgHash)
	...
	s.Require().Greater(balance.Uint64(), uint64(0))
	log.Logger.Info().Msgf("Spent %s nil", value.Sub(balance))
}

func (s *SuiteRpc) TestTopUpViaFaucet() {
	...
	receipt = s.waitForReceiptOnShard(types.MainWalletAddress.ShardId(), receipt.MsgHash)
	...
	testTopUp := func(initialValue, value, expectedValue uint64, delta float64) {
		...
		s.Require().NoError(err)
		receipt = s.waitForReceiptOnShard(address.ShardId(), mshHash)
		s.Require().NotNil(receipt)
		s.Require().True(receipt.Success)
		for _, r := range receipt.OutReceipts {
			s.Require().True(r.Success)
