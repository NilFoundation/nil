...
func (s *NilBlockGeneratorTestSuite) TestGetBlock() {
	smartAccountAdr, hexKey, err := CreateNewSmartAccount(s.logger)
	s.Require().NoError(err)
	s.Require().NotEqual("", smartAccountAdr)
	s.Require().NotEqual("", hexKey)

	contractAddress, err := DeployContract(smartAccountAdr, s.contractBasePath, hexKey, s.deployArgs, s.logger)
	s.Require().NoError(err)
	...
	blockHash, err := CallContract(smartAccountAdr, hexKey, calls, s.logger)
	s.Require().NoError(err)
