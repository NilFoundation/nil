...
func (bctx *buildContext) isEncoder(typ types.Type) bool {
	if types.Identical(typ, bctx.topType) {
		return false
	}
	return types.Implements(types.NewPointer(typ), bctx.encoderIface)
}
...
func (bctx *buildContext) isDecoder(typ types.Type) bool {
	...
	return types.Implements(types.NewPointer(typ), bctx.decoderIface)
}
...
func (bctx *buildContext) makeBasicOpAlias(named *types.Named, basic *types.Basic) (op, error) {
	op := basicOp{typ: named}
	kind := basic.Kind()
	switch {
	case kind == types.Bool:
		op.writeMethod = "WriteBool"
