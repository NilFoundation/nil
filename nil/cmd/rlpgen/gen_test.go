...
var (
	...
	testImporter   types.ImporterFrom
	testPackageRLP *types.Package
)

func init() {
	var ok bool
	testImporter, ok = importer.ForCompiler(testFset, "source", nil).(types.ImporterFrom)
	if !ok {
		panic("importer does not implement types.ImporterFrom")
	}
		...
		t.Run(test, func(t *testing.T) {
			...
			if os.Getenv("WRITE_TEST_FILES") != "" {
				if err := os.WriteFile(outputFile, output, 0o600); err != nil {
					t.Fatal("error writing test output:", err)
				}
			}
