...
func (s *SuiteFaucet) createWalletViaFaucet(ownerPrivateKey *ecdsa.PrivateKey, value int64) types.Address {
	...
	res := s.WaitForReceipt(s.client, types.BaseShardId, resHash)
...
func (s *SuiteFaucet) TestDeployContractViaFaucet() {
	...
	s.Require().NoError(err)
	receipt := s.WaitForReceipt(s.client, walletAddr.ShardId(), msgHash)
...
func (s *SuiteFaucet) TestTopUpViaFaucet() {
	...
	address, receipt := s.DeployContractViaMainWallet(s.client, types.BaseShardId, types.BuildDeployPayload(walletCode, common.EmptyHash), types.Value{})
	receipt = s.WaitForReceipt(s.client, types.MainWalletAddress.ShardId(), receipt.MsgHash)
	...
	testTopUp := func(initialValue, value, expectedValue uint64, delta float64) {
		...
		s.Require().NoError(err)
		receipt = s.WaitForReceipt(s.client, address.ShardId(), mshHash)
		s.Require().NotNil(receipt)
		s.Require().True(receipt.Success)
		for _, r := range receipt.OutReceipts {
			s.Require().True(r.Success)
