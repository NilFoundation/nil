...
func (s *WrapperTestSuite) TestFinalizedBatchIndex() {
	...
	expectedIndex := "42"
	s.callContractMock.AddExpectedCall("lastFinalizedBatchIndex", expectedIndex)
...
func (s *WrapperTestSuite) TestUpdateState_Success() {
	...
	s.callContractMock.AddExpectedCall("lastFinalizedBatchIndex", "41")
	s.callContractMock.AddExpectedCall("finalizedStateRoots", oldStateRoot)

	// Mock verifyDataProof calls (one per blob)
	...
	// Call method
	err := s.wrapper.UpdateState(s.ctx, batchIndex, oldStateRoot, newStateRoot, validityProof, publicDataInputs)
...
func (s *WrapperTestSuite) TestUpdateState_EmptyStateRoot() {
	...
	err := s.wrapper.UpdateState(
		s.ctx, batchIndex, emptyStateRoot, common.HexToHash("0x2222"), validityProof, publicDataInputs,
	)
	s.Require().Error(err)
	...
	err = s.wrapper.UpdateState(
		s.ctx, batchIndex, common.HexToHash("0x1111"), emptyStateRoot, validityProof, publicDataInputs,
	)
	s.Require().Error(err)
...
func (s *WrapperTestSuite) TestCommitBatch_Success() {
	...
	// Call method
	err := s.wrapper.CommitBatch(s.ctx, blobs, batchIndex)
...
func (s *WrapperTestSuite) TestNoopWrapper() {
	...
	// Test UpdateState
	err := noopWrapper.UpdateState(s.ctx, "42", common.Hash{}, common.Hash{}, nil, INilRollupPublicDataInfo{})
	s.Require().NoError(err)
