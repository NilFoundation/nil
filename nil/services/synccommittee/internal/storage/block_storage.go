...
func (bs *BlockStorage) validateBatchSequencing(
	...
	for shard, blockRef := range batch.EarliestRefs() {
		alreadyExists, err := bs.ops.blockExists(tx, blockRef.AsId())
		if err != nil {
			return err
		}
		if alreadyExists {
		...
		if err := currentRef.ValidateNext(blockRef); err != nil {
			return fmt.Errorf(
				"%w, batchId=%s, shardId=%s: latestFetched=%s, batchEarliest=%s",
				scTypes.ErrBatchMismatch, batch.Id, shard, currentRef, blockRef,
			)
		}
	}
