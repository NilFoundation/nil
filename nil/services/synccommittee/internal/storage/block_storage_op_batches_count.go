...
func newBatchCountOp(config BlockStorageConfig) batchCountOp {
	return batchCountOp{
		config: config,
	}
}

func (t batchCountOp) addStoredCount(tx db.RwTx, delta int32) error {
	...
	if newBatchesCount > t.config.StoredBatchesLimit {
		return fmt.Errorf(
			"%w: delta is %d, current storage size is %d, capacity limit is %d",
			ErrCapacityLimitReached, delta, currentBatchesCount, t.config.StoredBatchesLimit,
		)
	}
