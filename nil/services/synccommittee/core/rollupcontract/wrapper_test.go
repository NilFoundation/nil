...
func (s *WrapperTestSuite) TestNoopWrapper() {
	...
	// Test UpdateState
	updateStateData := testaide.NewUpdateStateData()
	err := noopWrapper.UpdateState(s.ctx, updateStateData)
	s.Require().NoError(err)

	// Test FinalizedBatchIndex
	index, err := noopWrapper.GetLatestFinalizedStateRoot(s.ctx)
	s.Require().NoError(err)
	s.Empty(index)

	// Test CommitBatch
	err = noopWrapper.CommitBatch(s.ctx, types.NewBatchId(), nil)
	s.Require().NoError(err)
}
