...
func NewCommitter(
	...
	return &committer{
		rollupClient: rollupClient,
		preparer:     NewCommitPreparer(encoder, builder, logger),
		clock:        clock,
		logger:       logger,
	}
}
...
func (c *committer) Commit(ctx context.Context, batch *types.BlockBatch) (committed *types.BlockBatch, err error) {
	...
	if err := c.rollupClient.VerifyDataProofs(ctx, commitment); err != nil {
		return nil, fmt.Errorf("data proofs verification failed: %w", err)
	}

	if err := c.rollupClient.CommitBatch(ctx, sealedBatch.Id, commitment.Sidecar); err != nil {
