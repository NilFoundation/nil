...
func (s *TracerNildTestSuite) getSingleBlockTraces(shardId types.ShardId, blockRef transport.BlockReference) *ExecutionTraces {
	s.T().Helper()
	var err error
	tracer := s.initTracer()
...
// If `multiBlock` is true, traces will be collected from range of blocks, otherwise, tracer will be called for each block individually.
	...
	for shardId, latestBlockNum := range latestBlocksForShards {
		if multiBlock {
			...
			for blockNum := from[shardId]; blockNum <= latestBlockNum; blockNum++ {
				blockIds = append(blockIds, BlockId{types.ShardId(shardId), transport.Uint64BlockReference(blockNum.Uint64())})
			}
