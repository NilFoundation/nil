...
func (s *TracerMockClientTestSuite) SetupTest() {
	...
	s.inMsgs = nil // remove messages from previous test
}
...
func (s *TracerMockClientTestSuite) addCallContractMessage(addr types.Address) {
	s.inMsgs = append(s.inMsgs, &types.Message{
...
func (s *TracerMockClientTestSuite) makeClient() client.Client {
	...
	cl.GetDebugContractFunc = func(contractAddr types.Address, blockId any) (*jsonrpc.DebugRPCContract, error) {
		s.T().Helper()
	...
	cl.GetDebugBlockFunc = func(shardId types.ShardId, blockId any, fullTx bool) (*jsonrpc.DebugRPCBlock, error) {
		s.T().Helper()
		block := &types.Block{
			Id:       1,
	...
	cl.GetBlockFunc = func(shardId types.ShardId, blockId any, fullTx bool) (*jsonrpc.RPCBlock, error) {
...
func (s *TracerMockClientTestSuite) TestStorageOps() {
	...
	traceData := s.simpleContractCallTrace(code)
	s.Require().Len(traceData.StorageOps, 4) // SSTORE produces two storage ops, read and write
	// TODO: clarify and fix extra storage op
	// s.Require().Equal(traceData.StorageOps[0].PC, traceData.StorageOps[1].PC)
	s.Require().False(traceData.StorageOps[1].IsRead)
	s.Require().True(traceData.StorageOps[2].IsRead)
	s.Require().True(traceData.StorageOps[3].IsRead)
}
