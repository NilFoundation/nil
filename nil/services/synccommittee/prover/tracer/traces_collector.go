...
func (tc *remoteTracesCollectorImpl) GetBlockTraces(
	...
	// Get previous block
	_, prevDbgBlock, err := tc.fetchAndDecodeBlock(ctx, blockId.ShardId, transport.BlockNumber(currentDbgBlock.Id-1).AsBlockReference())
	if err != nil {
		return nil, err
	}
	...
	if err := db.WriteBlock(tc.rwTx, blockId.ShardId, prevDbgBlock.Hash(blockId.ShardId), prevDbgBlock.Block); err != nil {
		return nil, fmt.Errorf("failed to write previous block to DB: %w", err)
	}

	// Initialize execution state, collect traces
	traces, err := tc.executeBlockAndCollectTraces(ctx, blockId.ShardId, currentDbgBlock, prevDbgBlock, configMap, gasPrices)
	if err != nil {
		return nil, err
	}
...
func (tc *remoteTracesCollectorImpl) executeBlockAndCollectTraces(
	...
	// Currently, MPT traces will contain only the last traced block. Since there is no MPT circuit yet, it's not a big deal.
	tc.initMptTracer(shardId, prevBlock.Id, prevBlock.SmartContractsRoot)
	// Initialize MPT tracer on first call
	// if tc.mptTracer == nil {
	// 	tc.initMptTracer(prevBlock.Id, prevBlock.SmartContractsRoot)
	// }
	// if tc.mptTracer.RootHash() != prevBlock.SmartContractsRoot {
	// 	return nil, fmt.Errorf("MPT tracer root hash (%s) doesn't match previous block's smart contracts root (%s)",
	// 		tc.mptTracer.RootHash(), prevBlock.SmartContractsRoot)
	// }
...
func (tc *remoteTracesCollectorImpl) getConfigForBlock(
	...
	blockWithConfigRaw, blockWithConfig, err := tc.fetchAndDecodeBlock(ctx, types.MainShardId, transport.HashBlockReference(blockWithConfigHash))
	if err != nil {
...
func (tc *remoteTracesCollectorImpl) collectGasPrices(
	...
	_, gasPricesBlock, err := tc.fetchAndDecodeBlock(ctx, types.MainShardId, transport.BlockNumber(blockWithGasPricesId).AsBlockReference())
	if err != nil {
		return nil, err
	}
...
// CollectTraces collects traces for blocks within the range specified in config. Traces are not written to a file, thus, `MarshalMode` and `BaseFileName`
// fields of the config are not used and could be omitted.
...
func CollectTraces(ctx context.Context, rpcClient api.RpcClient, cfg *TraceConfig) (*ExecutionTraces, error) {
	...
	// FIXME: MPT trace aggregates changes from multiple sequential blocks, and can't be constructed from multiple shards.
