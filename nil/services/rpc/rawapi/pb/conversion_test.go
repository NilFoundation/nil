...
func TestOutMessage_PackUnpack(t *testing.T) {
	...
	value := types.NewValueFromUint64(321)
	nestedMessage := &rpctypes.OutMessage{
		MessageSSZ:  hexutil.Bytes{0x11},
	...
	args := &rpctypes.OutMessage{
		MessageSSZ:  hexutil.Bytes{0x1},
...
func TestCallResponse_PackUnpack(t *testing.T) {
	...
	args := &rpctypes.CallResWithGasPrice{
		...
		CoinsUsed:   value,
		OutMessages: []*rpctypes.OutMessage{outMsg},
		GasPrice:    gp,
	}
