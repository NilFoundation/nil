...
func NewBlockAccessor(blocksLRUSize int) *BlockAccessor {
	return &BlockAccessor{
		...
		hashByNumber: NewBlockHashByNumberAccessor(blocksLRUSize),
	}
}
...
type BlockHashByNumberAccessor struct {
	cache *lru.Cache[shardAndBlockId, common.Hash]
}

func NewBlockHashByNumberAccessor(blocksLRUSize int) *BlockHashByNumberAccessor {
	cache, err := lru.New[shardAndBlockId, common.Hash](blocksLRUSize)
	check.PanicIfErr(err)
...
func (b *BlockHashByNumberAccessor) Get(tx db.RoTx, shardId types.ShardId, num types.BlockNumber) (common.Hash, error) {
	...
	hash, err := db.ReadBlockHashByNumber(tx, shardId, num)
	if err != nil {
		return common.EmptyHash, err
	}

	b.cache.Add(key, hash)
	return hash, nil
}
