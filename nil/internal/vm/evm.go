...
func (evm *EVM) Call(
	...
	if decoded, relayer, err := contracts.DecodeCallData(nil, input); err == nil {
		evm.StateDB.Logger().Debug().Msgf("  decoded: %s", decoded)
		evm.StateDB.Logger().Debug().Msgf("  relayer: %s", relayer)
	}

	if addr == types.HexToAddress("0x00024072511ba54dc4034b19023891fe73cea286") {
		evm.StateDB.Logger().Debug().Msgf("  AAAAA")
		evm.StateDB.EnableVmTracing()
	} else if evm.depth == 0 {
		evm.StateDB.DisableVmTracing()
	}
	...
	if isPrecompile {
		...
		if exists, err := evm.StateDB.Exists(addr); err != nil {
			...
			if value.IsZero() {
				//return nil, gas, fmt.Errorf("cannot call non-existing account %s with zero value", addr.Hex())
				// Calling a non-existing account, don't do anything.
				return nil, gas, fmt.Errorf("contract doesn't exist: %s", addr.Hex())
				//return nil, gas, nil
			}
		...
		if len(code) == 0 {
			...
			//return nil, gas, fmt.Errorf("code is empty %s", addr.Hex())
